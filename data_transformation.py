...
def lambda_handler(event, context):
    ...
    for files in s3.list_objects(Bucket=Bucket, Prefix=Key)['Contents']:
        ...
        if files_key.split('.')[-1] == "json":
            response = s3.get_object(Bucket=Bucket, Key=files_key)
    ...
    for data in spotify_data:
        ...
        #create dataframe for song
        song_df = pd.DataFrame.from_dict(song_list)
        song_df['song_added'] =  pd.to_datetime(song_df['song_added'])
        
        album_key = "transformed_data/albums_data/ablum_transformed_" +str(datetime.now())+ ".csv"
